...
export default function SupplierDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const r = await authClient.put(`/suppliers/${id}`, {
        name: form.name, taxId: form.taxId, currency: form.currency,
  ...
  const handleToggleActive = async () => {
    try {
      const r = await authClient.patch(`/suppliers/${id}/toggle-active`);
      setSupplier(r.data);
    } catch (ex) { setError(errMsg(ex)); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await authClient.delete(`/suppliers/${id}`);
  ...
  if (!supplier) return (
    <div className="mx-card p-6 flex items-center gap-2 text-[#FF3B30]">
      <AlertCircle size={16} /><span className="text-sm">{error || 'Proveedor no encontrado'}</span>
    </div>
  );
    ...
    <div className="mx-fade-in max-w-2xl">
      <div className="mx-page-header">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="mx-page-title">{supplier.name}</h1>
            <p className="text-xs text-[#86868B]">{supplier.taxId}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={handleToggleActive} className="mx-btn-secondary text-sm">
          ...
          {editing ? (
            ...
            <>
              ...
              <button onClick={handleDelete} className="mx-btn-danger text-sm">Eliminar</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="mx-card p-6">
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            { label: 'Teléfono', field: 'contactPhone' },
            { label: 'Plazo de pago (días)', field: 'paymentTermsDays' },
          ].map(({ label, field }) => (
            <div key={field}>
              <label className="block text-xs text-[#86868B] mb-1">{label}</label>
              {editing ? (
                <input className="mx-input" value={(form as any)[field] ?? ''} onChange={set(field)} />
              ) : (
                <p className="text-sm text-[#1D1D1F]">{(supplier as any)[field] || '—'}</p>
              )}
            </div>
          ))}
          <div>
            <label className="block text-xs text-[#86868B] mb-1">Estado</label>
            <span className={supplier.isActive ? 'mx-badge mx-badge-success' : 'mx-badge mx-badge-neutral'}>
