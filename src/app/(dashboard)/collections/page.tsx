...
interface Movement { id:string; bankAccountId:string; direction:string; amount:number; currency:string; movementDate:string; description:string; reference:string|null; matchedSaleId:string|null; matchedPurchaseId:string|null; }
    ...
    <div>
      ...
      {overview && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:20 }}>
          <div className="mx-kpi"><div className="mx-kpi-label">Ingresos del mes</div><div className="mx-kpi-value" style={{ color:'#34C759' }}>{formatCurrency(overview.totalInflows)}</div></div>
          <div className="mx-kpi"><div className="mx-kpi-label">Egresos del mes</div><div className="mx-kpi-value" style={{ color:'#FF3B30' }}>{formatCurrency(overview.totalOutflows)}</div></div>
          <div className="mx-kpi"><div className="mx-kpi-label">Flujo neto</div><div className="mx-kpi-value" style={{ color: overview.netFlow >= 0 ? '#34C759' : '#FF3B30' }}>{formatCurrency(overview.netFlow)}</div></div>
        </div>
      )}
      ...
      <div className="mx-card-section">
        ...
        {showForm && (
          <form onSubmit={handleCreate} style={{ padding:'14px 20px', background:'#F9F9FB', borderBottom:'0.5px solid #E5E5EA', display:'flex', gap:8 }}>
            <input className="mx-input" placeholder="Banco" value={form.bankName} onChange={e => setForm(p=>({...p,bankName:e.target.value}))} required style={{ flex:1 }}/>
            <input className="mx-input" placeholder="N° cuenta" value={form.accountNumber} onChange={e => setForm(p=>({...p,accountNumber:e.target.value}))} required style={{ flex:1 }}/>
            <select className="mx-select" value={form.currency} onChange={e => setForm(p=>({...p,currency:e.target.value}))} style={{ width:100 }}>
              <option value="PEN">PEN</option><option value="USD">USD</option>
            </select>
            <input type="number" className="mx-input" placeholder="Saldo inicial" value={form.initialBalance||''} onChange={e => setForm(p=>({...p,initialBalance:Number(e.target.value)}))} style={{ width:140 }}/>
            ...
            <tbody>
              {accounts.map(a => (
                <tr key={a.id}>
                  ...
                  <td style={{ fontWeight:600 }}>{formatCurrency(a.currentBalance, a.currency)}</td>
                  <td><span className={a.isActive ? 'mx-badge mx-badge-success' : 'mx-badge mx-badge-neutral'}>{a.isActive ? 'Activa' : 'Inactiva'}</span></td>
                  <td><button onClick={() => handleDelete(a.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'#FF3B30' }}><Trash2 size={14}/></button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function MovementsTab() {
  ...
  const [error,     setError]     = useState('');
  const [showForm,  setShowForm]  = useState(false);
  const [form, setForm] = useState({ bankAccountId:'', direction:'IN', amount:'', currency:'PEN', movementDate:new Date().toISOString().split('T')[0], description:'', reference:'' });
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:16 }}>
        <select className="mx-select" style={{ width:200 }} value={accountId} onChange={e => setAccountId(e.target.value)}>
      ...
      {showForm && (
        <div className="mx-form-card" style={{ marginBottom:16 }}>
          <form onSubmit={handleCreate} style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
            ...
            <div><label className="mx-label">Fecha *</label><input type="date" className="mx-input" value={form.movementDate} onChange={e => setForm(p=>({...p,movementDate:e.target.value}))} required/></div>
            ...
            <div><label className="mx-label">Referencia</label><input className="mx-input" value={form.reference} onChange={e => setForm(p=>({...p,reference:e.target.value}))}/></div>
      ...
      <div className="mx-card-section">
        {loading ? (
          <div style={{ padding:16, display:'flex', flexDirection:'column', gap:8 }}>{[1,2,3].map(i=><div key={i} className="mx-skeleton" style={{ height:44, borderRadius:8 }}/>)}</div>
        ) : movements.length === 0 ? (
          ...
          <table className="mx-table">
            ...
            <tbody>
              {movements.map(m => (
                <tr key={m.id}>
                  <td style={{ color:'#86868B' }}>{formatDate(m.movementDate)}</td>
                  <td>{m.description}</td>
                  ...
                  <td style={{ fontWeight:600, color: m.direction==='IN'?'#34C759':'#FF3B30' }}>{formatCurrency(m.amount, m.currency)}</td>
                  ...
                  <td>{!m.matchedSaleId&&!m.matchedPurchaseId&&<button onClick={()=>handleDelete(m.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'#FF3B30' }}><Trash2 size={14}/></button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        <div className="mx-card-section">
          ...
          {loading ? (
            <div style={{ padding:12, display:'flex', flexDirection:'column', gap:8 }}>{[1,2,3].map(i=><div key={i} className="mx-skeleton" style={{ height:44, borderRadius:8 }}/>)}</div>
          ) : movements.length === 0 ? (
            ...
            <div>
              {movements.map(m => (
                <button key={m.id} onClick={() => loadSuggestions(m.id)}
                  style={{ width:'100%', padding:'12px 20px', border:'none', background: selected===m.id ? '#EFF6FF' : 'transparent', cursor:'pointer', textAlign:'left', borderBottom:'0.5px solid #F2F2F7', fontFamily:'inherit' }}>
                  <p style={{ fontSize:13, fontWeight:500, color:'#1D1D1F' }}>{m.description}</p>
                  <p style={{ fontSize:12, color:'#86868B', marginTop:2 }}>{formatDate(m.movementDate)} · {formatCurrency(m.amount, m.currency)}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="mx-card-section">
          ...
          {loadingSugg ? (
            <div style={{ padding:12, display:'flex', flexDirection:'column', gap:8 }}>{[1,2].map(i=><div key={i} className="mx-skeleton" style={{ height:56, borderRadius:8 }}/>)}</div>
          ) : !selected ? (
            <div className="mx-empty" style={{ padding:32 }}>← Selecciona un movimiento</div>
              ...
              {suggestions.map((s:any) => (
                <div key={s.id} style={{ padding:'12px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'0.5px solid #F2F2F7' }}>
                  <div>
                    ...
                    <p style={{ fontSize:12, color:'#86868B' }}>{formatCurrency(s.totalAmount ?? s.amount)}{s.score ? ` · Score: ${s.score}` : ''}</p>
                  </div>
