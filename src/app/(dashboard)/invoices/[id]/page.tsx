...
export default function InvoiceDetailPage() {
  ...
  const handleIssue = async () => {
    setActing('issue');
    try {
      const r = await authClient.post(`/invoices/${id}/issue`);
      setInvoice(r.data);
    } catch (ex) { setError(errMsg(ex)); }
    finally { setActing(''); }
  };
  ...
  if (!invoice) return (
    <div className="mx-card p-6 flex items-center gap-2 text-[#FF3B30]">
      <AlertCircle size={16} /><span className="text-sm">{error || 'Factura no encontrada'}</span>
    </div>
  );
    ...
    <div className="mx-fade-in max-w-3xl">
      <div className="mx-page-header">
        <div className="flex items-center gap-3">
          <Link href="/invoices" className="mx-btn-secondary px-3 py-2">
            <ArrowLeft size={15} />
          </Link>
          <div>
            <h1 className="mx-page-title">{invoice.invoiceNumber}</h1>
            <p className="text-xs text-[#86868B]">{invoice.documentType}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className={STATUS_BADGE[invoice.status] ?? 'mx-badge mx-badge-neutral'}>
            {STATUS_LABEL[invoice.status] ?? invoice.status}
          </span>
          {canIssue && (
            <button onClick={handleIssue} disabled={!!acting} className="mx-btn-primary">
              {acting === 'issue' ? 'Emitiendo…' : 'Emitir'}
            </button>
          )}
          {canAnnul && (
            <button onClick={handleAnnul} disabled={!!acting} className="mx-btn-danger">
              {acting === 'annul' ? 'Anulando…' : 'Anular'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mx-card p-6 mb-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xs text-[#86868B]">{invoice.clientRucDni}</p>
          </div>
          <div>
            <p className="text-xs text-[#86868B] mb-1">Dirección</p>
            <p className="text-sm">{invoice.clientAddress || '—'}</p>
          </div>
          <div>
            <p className="text-xs text-[#86868B] mb-1">Fecha emisión</p>
            <p className="text-sm">{formatDate(invoice.invoiceDate)}</p>
          </div>
          <div>
            <p className="text-xs text-[#86868B] mb-1">Vencimiento</p>
            <p className="text-sm">{formatDate(invoice.dueDate)}</p>
          </div>
          <div>
            <p className="text-xs text-[#86868B] mb-1">Estado SUNAT</p>
            <p className="text-sm">{invoice.sunatStatus}</p>
            {invoice.sunatResponseDesc && (
              <p className="text-xs text-[#FF3B30] mt-0.5 line-clamp-2">{invoice.sunatResponseDesc}</p>
            )}
          </div>
          <div>
            <p className="text-xs text-[#86868B] mb-1">Asiento contable</p>
            <p className="text-sm">{invoice.journalEntryId ? '✓ Generado' : '—'}</p>
          </div>
          {invoice.notes && (
      ...
      <div className="mx-card overflow-hidden mb-4">
            ...
            {invoice.items.map(item => (
              <tr key={item.id}>
                ...
                <td>
                  ...
                  {item.productCode && <p className="text-xs text-[#86868B]">{item.productCode}</p>}
                </td>
        ...
        <div className="px-4 py-4 border-t border-[#E5E5EA] flex justify-end">
          <div className="space-y-1 min-w-48">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-[#86868B]">IGV ({Number(invoice.igvRate)}%)</span>
              <span>{formatCurrency(invoice.igvAmount, invoice.currency)}</span>
            </div>
            ...
            {Number(invoice.amountPaid) > 0 && (
              <>
                ...
                <div className="flex justify-between text-sm font-medium text-[#FF9F0A]">
                  <span>Saldo</span>
                  <span>{formatCurrency(Number(invoice.totalAmount) - Number(invoice.amountPaid), invoice.currency)}</span>
                </div>
              </>
            )}
          </div>
