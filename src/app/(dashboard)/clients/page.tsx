...
interface Client {
  ...
  phone: string; address: string; status: string; createdAt: string;
}
...
export default function ClientsPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(clients.filter(c =>
      c.name.toLowerCase().includes(q) || c.taxId.includes(q) || c.email.toLowerCase().includes(q)
    ));
  }, [search, clients]);
  ...
  return (
    <div className="mx-fade-in">
      ...
      <div className="mx-card p-3 mb-4 flex items-center gap-2">
        ...
        <input className="flex-1 outline-none text-sm bg-transparent placeholder:text-[#86868B]"
          placeholder="Buscar por nombre, RUC o email…" value={search} onChange={e => setSearch(e.target.value)} />
      </div>

      {error && <div className="mx-card p-4 flex items-center gap-2 text-[#FF3B30] mb-4"><AlertCircle size={16} /><span className="text-sm">{error}</span></div>}

      <div className="mx-card overflow-hidden">
        {loading ? (
          <div className="p-4 space-y-2">{[1,2,3,4,5].map(i => <div key={i} className="mx-skeleton h-12 rounded-lg" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="p-8 text-center text-sm text-[#86868B]">{search ? 'Sin resultados' : 'No hay clientes registrados'}</div>
        ) : (
          <table className="mx-table">
            <thead><tr><th>Nombre</th><th>RUC / DNI</th><th>Email</th><th>Teléfono</th><th>Estado</th><th>Creado</th></tr></thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id}>
                  <td><Link href={`/clients/${c.id}`} className="font-medium text-[#0071E3] hover:underline">{c.name}</Link></td>
      ...
      {!loading && filtered.length > 0 && <p className="text-xs text-[#86868B] mt-2 px-1">{filtered.length} cliente(s)</p>}
    </div>
  );
}
