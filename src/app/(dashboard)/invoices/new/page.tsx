...
interface Line {
  ...
  quantity: number; unitPrice: number; discountPct: number; subtotal: number;
}
...
function NewInvoiceForm() {
  ...
  useEffect(() => {
    authClient.get('/clients').then(r => setClients(r.data)).catch(() => {});
    // due date = 30 días
  ...
  const updateLine = (i: number, field: keyof Line, value: any) => {
    setLines(prev => {
      ...
      updated[i].subtotal = qty * price * (1 - disc / 100);
      return updated;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lines.some(l => !l.productName.trim())) { setError('Completa el nombre de todos los productos'); return; }
    setLoading(true);
    setError('');
    try {
    ...
    <div className="mx-fade-in max-w-3xl">
      <div className="mx-page-header">
        <div className="flex items-center gap-3">
          ...
          <h1 className="mx-page-title">Nueva factura</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="mx-card p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[#1D1D1F] mb-1">Fecha emisión *</label>
              <input type="date" className="mx-input" value={date} onChange={e => setDate(e.target.value)} required />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#1D1D1F] mb-1">Vencimiento *</label>
            ...
            {lines.map((line, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-end">
                <div className="col-span-4">
                  ...
                  <input className="mx-input" placeholder="Nombre del producto/servicio"
                ...
                <div className="col-span-1">
                  {i === 0 && <label className="block text-xs text-[#86868B] mb-1">Subtotal</label>}
                  <p className="text-sm font-medium pt-2">{formatCurrency(line.subtotal)}</p>
                </div>
                <div className="col-span-1 flex justify-end">
                  {lines.length > 1 && (
                    <button type="button" onClick={() => removeLine(i)}
                      className="text-[#FF3B30] hover:opacity-70 p-2">
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
        ...
        <div className="flex gap-3 justify-end">
          <Link href="/invoices" className="mx-btn-secondary">Cancelar</Link>
